...
df2 <- select(sarah_data, Speaker, Gloss, Date)
...
```{R}
sarah_data <- read_csv("Sarah_analysis3.csv")
...
```{R}
Speaker <- as.factor(sarah_data$Speaker)
class(Speaker)

Gloss <- as.character(sarah_data$Gloss)
class(Gloss)

Date <- anytime::anydate(sarah_data$Date)
class(Date)
```
...
```{R}
child_mother_df <- filter(sarah_data, c(Speaker == "MOT" | Speaker == "CHI"))
view(child_mother)
...
sarah_data$clean <- str_replace_all(sarah_data$Gloss,"[:punct:]","")
...
df2$num_words <- stri_count_words(df2$Gloss)
...
mlu_df <- summarise(df3, word_sum = sum(num_words), utterances = n(), mlu =  word_sum/utterances)

mlu_df2 <- na.omit(mlu_df)
...
```{R}
write.csv(mlu_df2,"Sarah_data_done.csv")
