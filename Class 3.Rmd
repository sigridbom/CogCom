...
```{R}
`''
knitr::opts_knit$set(root.dir = "Users/sigridnielsen/CogCom/Sarah_analysis/")
...
?list.files
setwd("/Users/sigridnielsen/CogCom/Sarah_analysis/")
sarah_data <- list.files(full.names = TRUE, all.files = TRUE)
sarah_data
sarah_data <- (sarah_data1 + all.variables)
names(sarah_data1)
sarah_data_org <- list.files(path="data",full.names=TRUE, all.files=TRUE)
sarah_now <- list.files(path = "data", full.names = TRUE, all.files = TRUE)
source("CRANtoR_function.R")
...
```{R}
sarah_data <- map_df(sarah_data,function(x) read.CLAN.file(x))
```
...
```{R}
sarah_data1 <- select(sarah_data, c(Speaker, Gloss, Date))
```
...
```{R}
write.csv(sarah_data1,"Sarah_analysis.csv")
```
###Grøn tekst = navn på fil, filen ligger ved working directory i mappen "data"
...
```{R}
sarah_data1 <- read_csv("Sarah_analysis/data/Sarah_analysis.csv")
...
```{R}
child_mother <- filter(sarah_data, c(Speaker == "MOT" | Speaker == "CHI"))
```
