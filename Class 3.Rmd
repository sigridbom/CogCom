...
?list.files
setwd("/Users/sigridnielsen/CogCom/")
sarah_data <- list.files(path = "data", full.names = TRUE, all.files = TRUE)

sarah_data <- sarah_data[-c(1,2)]
...
```{R}
sarah_data_df <- map_df(sarah_data,function(x)read.CLAN.file(x))
```
...
df2 <- select(sarah_data_df, Speaker, Gloss, Date)
...
```{R}
write.csv(sarah_data_df,"Sarah_analysis3.csv")
```
###Grøn tekst = navn på fil, filen ligger ved working directory i mappen "cogcom"
...
```{R}
sarah_data <- read_csv("Sarah_analysis/data/Sarah_analysis3.csv")
...
```{R}
Speaker <- as.factor(sarah_data1$Speaker)
class(Speaker)

Gloss <- as.character(sarah_data1$Gloss)
class(Gloss)

Date <- anytime::anydate(sarah_data1$Date)
class(Date)
```
...
```{R}
child_mother_df <- filter(sarah_data_df, c(Speaker == "MOT" | Speaker == "CHI"))
view(child_mother)

df2 <- filter(df2, c(Speaker == "MOT" | Speaker == "CHI"))
```
...
sarah_data$clean <- str_replace_all(sarah_data_df$Gloss,"[:punct:]","")

df2$clean <- str_replace_all(df2$Gloss,"[:punct:]","")
...
child_mother_df$num_words <- stri_count_words(child_mother_df$Gloss)

MLU <- child_mother_df$num_words/child_mother_df$num_utt
mean(MLU)

#3,433943 words on average

df2$num_words <- stri_count_words(df2$clean)
MLU2 <- df2$num_words/df2$num_utt
#Why did we have to make a new data frame with only sum columns when we need the num_utt column? 
```
...
df3 <- group_by(df2, df2$Speaker, df2$Date)
